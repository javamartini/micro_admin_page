...
async function gatherData() {
  ...
  if (navigator.geolocation) {
    //IF ALLOWED, will gather accurate location
    navigator.geolocation.getCurrentPosition(
      async function (position) {
        ...
        const objResponse = await fetch(
          `https://api.timezonedb.com/v2.1/get-time-zone?key=GSWUCIUB70IF&format=json&by=position&lat=${floLatitude}&lng=${floLongitude}`,
        );

        if (!objResponse.ok) {
          throw new Error(`HTTP Error Status: ${objResponse.status}`);
        } else {
          //if no errors, gather data
      ...
      async function error(err) {
        //IF DENIED, we will gather less accurate location via IP.
        console.warn(
          `ERROR(${err.code}): ${err.message}, switching to location by IP`,
        );
        Swal.fire({
          //notifying the user of location via IP
    ...
    //will play if the browser does not support geolocation
...
async function getWeatherData(floLatitude, floLongitude, strTimezone) {
  ...
  try {
    ...
    document.querySelector("#txtCurrentTemp").innerHTML =
      `<h4>${temp}${tempUnit}</h4>`;
    ...
    if (precipMeasure == "inch") {
      //this condition is purely for formatting purposes.
      document.querySelector("#txtCurrentPrecip").innerHTML =
        `<h4>${precip} inches</h4>`;
    } else {
      document.querySelector("#txtCurrentPrecip").innerHTML =
        `<h4>${precip} ${precipMeasure}</h4>`;
    }
    ...
    document.querySelector("#txtCurrentFeel").innerHTML =
      `<h4>${apparentTemp}${appTempFeel}</h4>`;

    //for relative humidity
    const humid = current.relative_humidity_2m;
    const humidMeasure = currentMeasure.relative_humidity_2m;
    document.querySelector("#txtCurrentHumid").innerHTML =
      `<h4>${humid}${humidMeasure}</h4>`;
  } catch (objError) {
    // if open-meteo is currently unavailable
...
function weatherCode(wmoCode) {
  ...
  switch (wmoCode) {
    case 0: //clear sky
      strHTML =
        "<i class='bi bi-sun-fill' style='font-size: 2rem; color: yellow;'></i><br>";
      strHTML += "<p text-center>clear skies</p>";
      break;
    case 1: //mainly clear
      strHTML =
        "<i class='bi bi-cloud-sun' style='font-size: 2rem; color: yellow;'></i><br>";
      strHTML += "<p text-center>mainly clear</p>";
      break;
    case 2: //partly cloudy
      strHTML =
        "<i class='bi bi-cloud-sun-fill' style='font-size: 2rem; color: gray;'></i><br>";
    ...
    case 45: //fog
      strHTML =
        "<i class='bi bi-cloud-fog-fill' style='font-size: 2rem; color: lightgray;'></i><br>";
    ...
    case 51: //light drizzle
      strHTML =
        "<i class='bi bi-cloud-drizzle' style='font-size: 2rem; color: lightskyblue;'></i><br>";
    ...
    case 56: //light freezing drizzle
      strHTML =
        "<i class='bi bi-cloud-drizzle-fill' style='font-size: 2rem; color: lightblue;'></i><br>";
    ...
    case 61: //slight rain
      strHTML =
        "<i class='bi bi-cloud-rain' style='font-size: 2rem; color: darkblue;'></i><br>";
    ...
    case 65: //heavy rain
      strHTML =
        "<i class='bi bi-cloud-rain-heavy' style='font-size: 2rem; color: darkblue;'></i><br>";
      strHTML += "<p text-center>heavy rain</p>";
      break;
    case 66: //light freezing rain
      strHTML =
        "<i class='bi bi-cloud-rain-fill' style='font-size: 2rem; color: blue;'></i><br>";
      strHTML += "<p text-center>light freezing rain</p>";
      break;
    case 67: //heavy freezing rain //halfway through this and im about to cry
      strHTML =
        "<i class='bi bi-cloud-rain-heavy-fill' style='font-size: 2rem; color: blue;'></i><br>";
      strHTML += "<p text-center>heavy freezing rain</p>";
      break;
    case 71: //slight snow fall //i swear theres a better way to do this
      strHTML =
        "<i class='bi bi-cloud-snow' style='font-size: 2rem; color: lightskyblue;'></i><br>";
    ...
    case 75: //heavy snow fall
      strHTML =
        "<i class='bi bi-cloud-snow-fill' style='font-size: 2rem; color: lightskyblue;'></i><br>";
    ...
    case 95: //slight or moderate thunderstorm
      strHTML =
        "<i class='bi bi-cloud-lightning-rain-fill' style='font-size: 2rem; color: darkblue;'></i><br>";
...
function setUserTempSettings() {
  ...
  if (strTempMeasure == "fahrenheit") {
    //if fahrenheit, change to celsius
    strTempMeasure = "celsius";
  } else {
    //if celsius, change to fahrenheit
    strTempMeasure = "fahrenheit";
  }

  if (strTempMeasure == "fahrenheit" || strTempMeasure == "celsius") {
    Swal.fire({
      //letting the user know their selection is saved
      position: "center",
      icon: "success",
    ...
    //shouldn't happen, but just in case, it is still important feedback
    Swal.fire({
      position: "center",
      icon: "error",
  ...
  document.querySelector("#txtTempMeasure").innerHTML =
    `<span style="color: red;">${localStorage.getItem("tempMeasure")}</span>`;
}
...
function setUserPrecipSettings() {
  ...
  if (strPrecipMeasure == "inch") {
    //if inches, change to millimeters
    strPrecipMeasure = "mm";
  } else {
    //if millimeters, change to inches
  ...
  //outputting their current selection to the website
  document.querySelector("#txtPrecipMeasure").innerHTML =
    `<span style="color: red;">${localStorage.getItem("precipMeasure")}</span>`;
}

//will grab user settings. if none, then it will use create default values for local storage
function getUserSettings() {
  let strTempMeasure = localStorage.getItem("tempMeasure");
  if (strTempMeasure === null) {
    //=== for equal value and equal type
